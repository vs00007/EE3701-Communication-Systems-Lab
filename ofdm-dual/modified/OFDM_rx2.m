function [M_n,Threshold_graph,H_est_time,RX_Payload_1_no_Equalizer,RX_Payload_2_no_Equalizer,RX_Payload_1_no_pilot,RX_Payload_2_no_pilot,BER, H_est] = OFDM_rx2(RX,Parameters_struct)
%% Debug mode
Debug_mode = 'off';
...
% function [M_n,Threshold_graph,H_est_time,RX_Payload_1_no_Equalizer,RX_Payload_2_no_Equalizer,RX_Payload_1_no_pilot,RX_Payload_2_no_pilot,BER] = OFDM_rx2(RX,Parameters_struct)
% %% Debug mode
% Debug_mode = 'off';
% if strcmp(Debug_mode,'on')
%    clearvars -except Debug_mode;close all;clc;
...
% for n=1:length(RX)-D+1-L
%     for k=1:L
%         C_k(k) = RX(n+k-1)*conj(RX(n+k-1+D));
%         P_k(k) = abs(RX(n+k-1+D))^2;
%     end
%     C_n(n) = sum(C_k);
%     P_n(n) = sum(P_k);
...
%     if x < length(Packet_Front_idx)
%         if M_n(Packet_Front_idx(x)+Length_over_Threshold)>Threshold
%             idx = Packet_Front_idx(x)+L_RRC+1;
%             break; % Exit after finding first valid packet
%         end % if Loop
%     else
%         % Use the last packet front if no valid packet found
%         idx = Packet_Front_idx(x)+L_RRC+1;
%     end
% end % for Loop
% 
% % Safety check: ensure idx is set
% if ~exist('idx','var')
%     if ~isempty(Packet_Front_idx)
%         idx = Packet_Front_idx(1)+L_RRC+1;
%     else
%         error('No packet detected. Check threshold or input signal.');
%     end
% end
% Threshold_graph = Threshold*ones(1,length(M_n));
% Threshold_graph(idx-L_RRC-1) = 1.15;
...
% Frame_After_Fine = Frame_After_Coarse.*exp(-j*2*pi*f_Fine_est*Parameters_struct.Ts*(0:480-1)); % [1x160]
% %% Symbol Timing Estimation
% %% Initial Channel Estimation using Long Preambles
% Long_preamble_1 = Frame_After_Fine(Short_preamble_slot_length*12+1:Short_preamble_slot_length*16); % [1x64]
% Long_preamble_2 = Frame_After_Fine(Short_preamble_slot_length*16+1:Short_preamble_slot_length*20); % [1x64]
% Long_preamble_1_After_FFT = fftshift(fft(Long_preamble_1)); % [1x64]
% Long_preamble_2_After_FFT = fftshift(fft(Long_preamble_2)); % [1x64]
% 
% % Initial channel estimate from preambles
% H_est_preamble = 0.5*(Long_preamble_1_After_FFT+Long_preamble_2_After_FFT).*conj(Parameters_struct.Long_preamble_slot_Frequency); % [1x64]
% 
% %% 4-Tap Channel Estimation using Matrix Inversion Method
% % System parameters
% N_FFT = 64;
% L_channel = 4; % Number of channel taps (given assumption)
% 
% % Pilot subcarrier positions and known symbols
% pilot_positions = [12, 26, 40, 54]; % Pilot positions (1-indexed)
% pilot_symbols = [1, 1, 1, -1]; % Known pilot symbols
% 
% % Create DFT matrix for pilot positions and 4 taps
% % X matrix: pilot_symbols on diagonal
% X = diag(pilot_symbols); % [4x4]
% 
% % Create frequency response matrix for 4 taps at pilot positions
% % Each row corresponds to a pilot subcarrier, each column to a channel tap
% W = zeros(4, 4); % [4x4] matrix
% for p = 1:4
%     pilot_idx = pilot_positions(p) - 1; % Convert to 0-indexed for DFT
%     % Convert to normalized frequency index (-32 to +31 for 64-point FFT)
%     k = pilot_idx - N_FFT/2;
%     for tap = 0:3
%         W(p, tap+1) = exp(-1j * 2 * pi * k * tap / N_FFT);
%     end
% end
% 
% %% Channel Estimation and Equalization for Payload 1
% RX_Payload_1_time = Frame_After_Fine(320+1:400); % [1x80]
% RX_Payload_1_no_CP = RX_Payload_1_time(17:end); % [1x64]
% RX_Payload_1_Frequency = fftshift(fft(RX_Payload_1_no_CP)); % [1x64]
% 
% % Extract received pilots: Y = [Y(1), Y(3), Y(5), Y(7)]^T
% Y_pilots_1 = RX_Payload_1_Frequency(pilot_positions).'; % [4x1] column vector
% 
% % Solve for channel taps: h = (X^H * W)^(-1) * X^H * Y
% % Following the procedure from your image: h = X^(-1) * W^(-1) * Y
% XW_matrix = X * W; % [4x4]
% h_est_1 = XW_matrix \ Y_pilots_1; % [4x1] - Estimated channel taps
% 
% % Calculate frequency response for all subcarriers using estimated taps
% H_freq_1 = zeros(1, N_FFT);
% for sub = 1:N_FFT
%     k = sub - 1 - N_FFT/2; % Convert to normalized frequency index
%     H_freq_1(sub) = 0;
%     for tap = 0:3
%         H_freq_1(sub) = H_freq_1(sub) + h_est_1(tap+1) * exp(-1j * 2 * pi * k * tap / N_FFT);
%     end
% end
% 
% % Zero-forcing equalization
% RX_Payload_1_Frequency_Equalizer = RX_Payload_1_Frequency ./ H_freq_1; % [1x64]
% 
% %% Channel Estimation and Equalization for Payload 2
% RX_Payload_2_time = Frame_After_Fine(400+1:480); % [1x80]
% RX_Payload_2_no_CP = RX_Payload_2_time(17:end); % [1x64]
% RX_Payload_2_Frequency = fftshift(fft(RX_Payload_2_no_CP)); % [1x64]
% 
% % Extract received pilots
% Y_pilots_2 = RX_Payload_2_Frequency(pilot_positions).'; % [4x1] column vector
% 
% % Solve for channel taps
% h_est_2 = XW_matrix \ Y_pilots_2; % [4x1] - Estimated channel taps
% 
% % Calculate frequency response for all subcarriers
% H_freq_2 = zeros(1, N_FFT);
% for sub = 1:N_FFT
%     k = sub - 1 - N_FFT/2; % Convert to normalized frequency index
%     H_freq_2(sub) = 0;
%     for tap = 0:3
%         H_freq_2(sub) = H_freq_2(sub) + h_est_2(tap+1) * exp(-1j * 2 * pi * k * tap / N_FFT);
%     end
% end
% 
% % Zero-forcing equalization
% RX_Payload_2_Frequency_Equalizer = RX_Payload_2_Frequency ./ H_freq_2; % [1x64]
% 
% %% Average channel estimates for better performance
% h_est_avg = 0.5 * (h_est_1 + h_est_2); % [4x1] - Average channel taps
% H_est_time = [h_est_avg.', zeros(1, N_FFT-4)]; % [1x64] - Time domain representation
% 
% %% De-Mapping (Remove pilot and virtual subcarriers)
% % Data subcarrier positions: [7:11, 13:25, 27:32, 34:39, 41:53, 55:59]
% % Pilot positions: [12, 26, 40, 54] are excluded
% % DC subcarrier at position 33 is excluded
% % Virtual subcarriers at [1:6, 60:64] are excluded
% 
% RX_Payload_1_no_Equalizer = [RX_Payload_1_Frequency(7:11),RX_Payload_1_Frequency(13:25),RX_Payload_1_Frequency(27:32),RX_Payload_1_Frequency(34:39),RX_Payload_1_Frequency(41:53),RX_Payload_1_Frequency(55:59)]; % [1x48]
% RX_Payload_1_no_pilot = [RX_Payload_1_Frequency_Equalizer(7:11),RX_Payload_1_Frequency_Equalizer(13:25),RX_Payload_1_Frequency_Equalizer(27:32),RX_Payload_1_Frequency_Equalizer(34:39),RX_Payload_1_Frequency_Equalizer(41:53),RX_Payload_1_Frequency_Equalizer(55:59)]; % [1x48]
% RX_Payload_1_Final = pskdemod(RX_Payload_1_no_pilot,4,pi/4); % [1x48]
...
%     %--------------------------------------------------------------------------------%
%     subplot(2,4,5),plot(1:length(M_n),M_n,1:length(M_n),Threshold_graph);title('Packet Detection');axis([1,length(M_n),0,1.2]);axis square;
%     subplot(2,4,6),plot(abs(H_est_time));title('4-Tap Channel Estimation');axis([1 64 0 7]);axis square;xlabel('Time');
%     %--------------------------------------------------------------------------------%
%     subplot(2,4,7),plot(RX_Payload_1_no_Equalizer,'*');
%     hold on
%     subplot(2,4,7),plot(RX_Payload_2_no_Equalizer,'*');
...
%     hold on
%     subplot(2,4,8),plot(RX_Payload_2_no_pilot,'*');
%     title({'4-Tap Matrix Inversion';['BER = ',num2str(BER)]});axis([-1.5 1.5 -1.5 1.5]);axis square;
%     hold off
%     set(gcf,'Units','centimeters','position',[1 2 49 24]);
% end % Plot end
% %% End function
