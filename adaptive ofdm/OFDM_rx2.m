function [M_n,Threshold_graph,H_est_time,RX_Payload_1_no_Equalizer,RX_Payload_2_no_Equalizer,RX_Payload_1_no_pilot,RX_Payload_2_no_pilot, RX_Payload_3_no_Equalizer,RX_Payload_3_no_pilot,BER, H_est, packet_detected] = OFDM_rx2(RX,Parameters_struct)
%% Debug mode
Debug_mode = 'off';
...
%% Downsampling
Frame_DWN_sampling = RX_signal(idx:OVR:OVR*fsize+idx-1); % [1xfsize] Frame length
...
Frame_After_Coarse = Frame_DWN_sampling.*exp(-j*2*pi*f_Coarse_est*Parameters_struct.Ts*(0:fsize-1)); % [1xfsize]
%% Fine CFO Estimation
...
Frame_After_Fine = Frame_After_Coarse.*exp(-j*2*pi*f_Fine_est*Parameters_struct.Ts*(0:fsize-1)); % [1x160]
...
[RX_Payload_1_Frequency, RX_Payload_1_Frequency_Equalizer, RX_Payload_1_no_Equalizer, RX_Payload_1_no_pilot, RX_Payload_1_Final, RX_Payload_1_Final_BPSK, h_est_1, H_est_recon_1] = ...
    process_payload(RX_Payload_1_time, Nfft, Lch, pilot_idx, pilot_symbols);


    % --- Payload symbol 2 ---
RX_Payload_2_time = Frame_After_Fine(400+1:480); % [1x80]
[RX_Payload_2_Frequency, RX_Payload_2_Frequency_Equalizer, RX_Payload_2_no_Equalizer, RX_Payload_2_no_pilot, RX_Payload_2_Final, RX_Payload_2_Final_BPSK, h_est_2, H_est_recon_2] = ...
    process_payload(RX_Payload_2_time, Nfft, Lch, pilot_idx, pilot_symbols);

% disp("reached")
        % --- Payload symbol 3 ---
RX_Payload_3_time = Frame_After_Fine(480+1:560); % [1x80]
[RX_Payload_3_Frequency, RX_Payload_3_Frequency_Equalizer, RX_Payload_3_no_Equalizer, RX_Payload_3_no_pilot, RX_Payload_3_Final, RX_Payload_3_Final_BPSK, h_est_3, H_est_recon_3] = ...
    process_payload(RX_Payload_3_time, Nfft, Lch, pilot_idx, pilot_symbols);
...
% RX_Payload_3_no_Equalizer = [RX_Payload_3_Frequency(7:11),RX_Payload_3_Frequency(13:25),RX_Payload_3_Frequency(27:32),RX_Payload_3_Frequency(34:39),RX_Payload_3_Frequency(41:53),RX_Payload_3_Frequency(55:59)]; % [1x48]
% RX_Payload_3_no_pilot = [RX_Payload_3_Frequency_Equalizer(7:11),RX_Payload_3_Frequency_Equalizer(13:25),RX_Payload_3_Frequency_Equalizer(27:32),RX_Payload_3_Frequency_Equalizer(34:39),RX_Payload_3_Frequency_Equalizer(41:53),RX_Payload_2_Frequency_Equalizer(55:59)]; % [1x48]
% RX_Payload_3_Final = pskdemod(RX_Payload_3_no_pilot,4,pi/4); % [1x48]
% RX_Payload_3_Final_BPSK = pskdemod(RX_Payload_3_no_pilot,2);

% NumPayloads = 48;                   % e.g., 4 payload symbols per frame
% payload_len = 48;                  % each OFDM symbol = 80 samples (64 + 16 CP)
% Nfft = 64;
% Lch = 4;
% pilot_idx = [12, 26, 40, 54];
...
% demod_bits_BPSK_all = [];
% 
% % Starting sample index (after preambles)
% start_idx = 320;   % same as in your code
% 
% for p = 1:NumPayloads
%     % Extract current payload (each 80 samples)
%     payload_start = start_idx + (p-1)*payload_len + 1;
%     payload_end   = payload_start + payload_len - 1;
%     RX_Payload_time = Frame_After_Fine(payload_start:payload_end);
% 
%     % Call your function
%     [~, ~, ~, ~, demodQPSK, demodBPSK, h_est{p}, H_est_recon{p}] = ...
%         process_payload(RX_Payload_time, Nfft, Lch, pilot_idx, pilot_symbols);
% 
%     % Store final demodulated bits (each [1x48])
%     demod_bits_QPSK_all = [demod_bits_QPSK_all, demodQPSK];
%     demod_bits_BPSK_all = [demod_bits_BPSK_all, demodBPSK];
% end
% 
% tf = decode_Trollface(demod_bits_BPSK_all);
% 
% disp("died here");

% Now you have:
% demod_bits_QPSK_all : 1 × (48 * NumPayloads)
% demod_bits_BPSK_all : 1 × (48 * NumPayloads)

% disp('Total decoded QPSK bits:'), disp(length(demod_bits_QPSK_all));
% disp('Total decoded BPSK bits:'), disp(length(demod_bits_BPSK_all));


save finals.mat RX_Payload_3_Final RX_Payload_2_Final RX_Payload_1_Final;
%% BER calculation
Error_bits_QPSK = sum([abs(sign(Parameters_struct.data_Payload_1-RX_Payload_1_Final)),abs(sign(Parameters_struct.data_Payload_2-RX_Payload_2_Final)), abs(sign(Parameters_struct.data_Payload_1-RX_Payload_3_Final))]);
BER_QPSK = Error_bits_QPSK/(2*length(Parameters_struct.data_Payload_1)+length(Parameters_struct.data_Payload_2));

Error_bits_BPSK = sum([abs(sign(Parameters_struct.data_Payload_1_BPSK-RX_Payload_1_Final_BPSK)),abs(sign(Parameters_struct.data_Payload_2_BPSK-RX_Payload_2_Final_BPSK)), abs(sign(Parameters_struct.data_Payload_1_BPSK-RX_Payload_3_Final_BPSK))]);
BER_BPSK = Error_bits_BPSK/(2*length(Parameters_struct.data_Payload_1)+length(Parameters_struct.data_Payload_2));
BER_threshold = .01;
